...
for i=1:M
    ...
    S =sum(target(iz)./fx(iz));   % fill here sum of (target/pdf)
    data_uni(i) = S/N;
end
...
for i=1:M
    ...
    iz = find(abs(x)<=3);
    target = exp(-x.^2/2/sig^2)/sqrt(2*pi)/sig;
    S = sum(target(iz)./fx(iz));    % fill here
    data_nor(i) = S/N;
end
...
xlabel('estimated value','fontsize',14,'fontname','Helvetica');
ylabel('Histogram','fontsize',14,'fontname','Helvetica');
title(['True=',num2str(truevalue),', est by uniform rv=',num2str(estimation1),...
', var=',num2str(variance1),' (410586010)'],'fontsize',14,'fontname','Helvetica');
fname = 'prob_02_a03_uni(410586010).png';
print (fname, '-dpng'); 
...
', var=',num2str(variance2),' (410586010)'],'fontsize',14,'fontname','Helvetica');
fname = 'prob_02_a03_nor(410586010).png';
print (fname, '-dpng'); 
